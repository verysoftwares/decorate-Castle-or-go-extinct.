...
function flux:to(wtf, obj, time, vars)
  -- ?? there's an extra undocumented argument coming here ??
...
-- modifications to rxi's code
  function flux:finishall()
    for i = #self, 1, -1 do
      local t = self[i]
      for k, v in pairs(t.vars) do
        -- insta-set 0..1 progress to 1
          t.obj[k] = v.start + 1 * v.diff
      end
      flux.remove(self, i)
      if t._oncomplete then t._oncomplete(t.obj) end
    end
  ...
  function flux:active()
    -- ?? #self is always 0 here but not in finishall ??
    return #self>0
  end
...
local bound = {
  ...
  finishall = function(...) return flux.finishall(flux.tweens, ...) end,
}
